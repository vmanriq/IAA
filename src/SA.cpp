    ...
    for (auto cam = sol->camiones.begin(); cam != sol->camiones.end(); cam++)
    {
        ...
        for (int pos_nod = 0; pos_nod < (int)cam->ruta.size(); pos_nod++)
        {
            ...
            else
            {
                ...
                pond_fit += (inst.alpha) * inst.normRiesgos[riesgo][aux_cam.ruta[pos_nod - 1]][aux_cam.ruta[pos_nod]] + (1 - inst.alpha) * inst.normDistancias[riesgo][aux_cam.ruta[pos_nod - 1]][aux_cam.ruta[pos_nod]];
            }
        }
        idx_cam++;
    }
